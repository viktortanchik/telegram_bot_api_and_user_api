...
async def send_all_chanel(call: types.CallbackQuery):
    ...
    # texts = 'Original channel name ' +sql_select_original_channel_name(con,str(ids)) + '\n Previous channel name '+sql_select_previous_channel_names(con,str(ids))+ '\n Date of submission to the bot ' + sql_select_Date_of_submission_to_bot(con,str(ids))+'\n  Has a picture changed:  '+ picture_changed +'\n How many times: '+sql_select_How_many_times(con,str(ids))+"\n Current number of users in the channel: "+sql_select_Current_number_of_users_in_channel(con,str(ids))+'\n Number of users at the moment of insertion into the bot: '+ sql_select_number_of_users_at_the_moment_of_insertion_into_the_bot(con,str(ids))
    # print(texts)
    #task1 = asyncio.create_task('Original channel name ' +sql_select_original_channel_name(con,str(ids)) + '\n Previous channel name '+sql_select_previous_channel_names(con,str(ids))+ '\n Date of submission to the bot ' + sql_select_Date_of_submission_to_bot(con,str(ids))+'\n  Has a picture changed:  '+ picture_changed +'\n How many times: '+sql_select_How_many_times(con,str(ids))+"\n Current number of users in the channel: "+sql_select_Current_number_of_users_in_channel(con,str(ids))+'\n Number of users at the moment of insertion into the bot: '+ sql_select_number_of_users_at_the_moment_of_insertion_into_the_bot(con,str(ids)))
    #await task1
    #task1 = asyncio.create_task(texts)


    #await call.answer(text=texts, show_alert=True)
    buttons_clear()
