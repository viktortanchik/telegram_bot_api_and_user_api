# Нужно добавить возможноить получать имя канала при добавлении его в базу.
...
async def get_chat_name_on_url(delay,url):
    ...
    #await channel_full_info
    chat_id = asyncio.create_task(channel_full_info.full_chat.id)
    await chat_id
    chanPeer = asyncio.create_task(PeerChannel(channel_id=chat_id))
    await chanPeer
    channel_entity = asyncio.create_task(client.get_entity(chanPeer))
    await channel_entity
...
async def get_all_users(delay,channel):
    ...
    channel_full_info = loop.create_task(client(GetFullChannelRequest(channel=channel)))
    #print("channel_full_info: ",await channel_full_info)
    print("channel_full_info",await channel_full_info)

    async def testas(event):
    ...
    task3 = asyncio.create_task(testas(channel))
    await task3
# Возможно стоит отправлять сообщения на второго бота что бы он потом добавлял сам имя чата
    ...
    async def handler_all(event):
        ...
        # chat_id = loop.create_task(await channel_full_info.full_chat.id)
    # print(await chat_id)
    # #await chat_id
    #
    # chanPeer = asyncio.create_task(PeerChannel(channel_id=await chat_id))
    # channel_entity = asyncio.create_task(client.get_entity(await chanPeer))
    # print(await channel_entity.title)
